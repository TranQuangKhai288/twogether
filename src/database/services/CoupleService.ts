...
export class CoupleService {
  ...
  async createCouple(
    ...
    try {
      ...
      const partner = await this.userService.findByEmail(partnerEmail);
      if (!partner) {
        throw new AppError("Partner not found with this email", 404);
      }

      // Check if creator exists
      const creator = await this.userService.findById(creatorId.toString());
      if (!creator) {
      ...
      await Promise.all([
        this.userService.setUserCouple(creator._id.toString(), couple._id),
        this.userService.setUserCouple(partner._id.toString(), couple._id),
      ]);
  ...
  async joinCoupleByInviteCode(
    ...
    try {
      ...
      await this.userService.setUserCouple(userId, couple._id);
  ...
  async removeUserFromCouple(
    ...
    try {
      ...
      await this.userService.removeFromCouple(userId);
    } catch (error) {
      if (error instanceof AppError) throw error;
  ...
  async deleteCouple(coupleId: string | Types.ObjectId): Promise<void> {
    try {
      ...
      await Promise.all(
        couple.users.map((userId) => this.userService.removeFromCouple(userId))
      );
