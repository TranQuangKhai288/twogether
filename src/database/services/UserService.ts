...
export class UserService {
  ...
  async findByEmail(email: string): Promise<IUser | null> {
    try {
      return await User.findOne({ email: email.toLowerCase() }).populate('coupleId');
    } catch (error) {
   ...
   * Set user couple
   */
  async setUserCouple(userId: string | Types.ObjectId, coupleId: Types.ObjectId): Promise<IUser | null> {
    try {
      const user = await User.findByIdAndUpdate(
        userId,
        { $set: { coupleId } },
   ...
   */
  async removeFromCouple(userId: string | Types.ObjectId): Promise<IUser | null> {
    try {
      const user = await User.findByIdAndUpdate(
        userId,
        { $unset: { coupleId: '' } },
      ...
      if (user.coupleId) {
        ...
        if (couple) {
          ...
          if (couple.users.length === 0) {
            ...
            await User.findByIdAndUpdate(couple.users[0], { $unset: { coupleId: '' } });
          }
        }
      }
